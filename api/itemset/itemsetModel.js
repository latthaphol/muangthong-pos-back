        ...
        try {
            const productDetails = await knex('product')
                .select(
                    ...
                    knex.raw('(SELECT MIN(add_date) FROM product_lot WHERE product_lot.product_id = product.product_id AND product_lot.is_active = ?) as lot_add_date', [1])
                )
                .leftJoin('product_lot', 'product.product_id', '=', 'product_lot.product_id')
