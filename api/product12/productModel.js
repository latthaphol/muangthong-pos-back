...
class productModel {
    ...
    async return_product(order_id) {
        try {
            // 1. ดึงข้อมูล order จากฐานข้อมูล
            const orderProducts = await knex('order_products')
                .where({ order_id })
                .forUpdate(); // Lock the selected rows for update
            
            if (!orderProducts || orderProducts.length === 0) {
                throw new Error('No order products found for the given order');
            }
            
            // Calculate the total quantity of returned products
            const totalReturnedQuantity = orderProducts.reduce((total, product) => total + product.quantity, 0);
            
            // 2. ทำการคืนสินค้า (เพิ่มสินค้ากลับไปยังสต็อกหรือทำตามต้องการ)
            // Adjust the logic based on your specific requirements
            // For example, update the product quantities in the 'product' table
            await knex.transaction(async (trx) => {
                for (const product of orderProducts) {
                    await knex('product')
                        .where({ product_id: product.product_id })
                        .increment('product_qty', product.quantity)
                        .transacting(trx);
                }
                
                // 3. อัปเดตสถานะ order_products เป็น 'refund' หรือตามที่คุณต้องการ
                await knex('order_products')
                    .update({ status: 'refund' })
                    .where({ order_id })
                    .transacting(trx);
    
                // 4. อัปเดตสถานะในตาราง order เป็น 'refund' หรือตามที่คุณต้องการ
                await knex('order')
                    .update({ status: 'refund' })
                    .where({ order_id })
                    .transacting(trx);
